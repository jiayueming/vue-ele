<template>
    <div class="goods">
      <div class="menu-wrapper" ref="menuWrapper">
        <ul>
          <li v-for="(item, index) in goods" class="menu-item" @click="selectMenu(index, $event)" :key="index" :class="{'current':currentIndex === index}">
...
export default {
  ...
  created () {
    ...
    NetWorkUtils.get('/api/goods').then((response) => {
      let res = response
      if (res.errno === ERR_OK) {
  ...
  methods: {
    ...
    selectFood (food, event) {
      if (!event._constructed) { // pc防止两次点击
        return
      }
...
.goods
  ...
  overflow hidden
  .menu-wrapper
    flex 0 0 80px // flex属性，即不放大和缩小,80px为固定空间
    width 80px
    background #f3f5f7
    .menu-item
      display table
